...
> **This branch is the Bulgarian (`bulgarian`) language version.**
...
should print `bulgarian` for the Bulgarian Pi).
...
git fetch origin
git checkout bulgarian    # only needed the first time / if on the wrong branch
...
```bash
git fetch origin
git reset --hard origin/bulgarian
```
...
ssh pi@<PI_IP_ADDRESS>
cd ~/sound-clash
git fetch origin && git reset --hard origin/bulgarian
